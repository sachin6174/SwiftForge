...
func processImage(inputPath: String, outputPath: String) {
    ...
    print("📷 Processing iPad SS: \(inputPath) (\(pixelWidth)x\(pixelHeight))")
...
// Get files from ipad-ss/original directory
let fileManager = FileManager.default
let currentDir = fileManager.currentDirectoryPath
let ipadSsOriginalDir = "\(currentDir)/ipad-ss/original"
let ipadSsDir = "\(currentDir)/ipad-ss"

do {
    let files = try fileManager.contentsOfDirectory(atPath: ipadSsOriginalDir)
    let pngFiles = files.filter { $0.hasSuffix(".png") }.sorted()
    
    if pngFiles.isEmpty {
        print("No PNG screenshots found in \(ipadSsOriginalDir)")
    } else {
        for file in pngFiles {
            let inputPath = "\(ipadSsOriginalDir)/\(file)"
            let outputPath = "\(ipadSsDir)/\(file)"
            processImage(inputPath: inputPath, outputPath: outputPath)
        }
        print("🎉 Finished processing all iPad screenshots successfully!")
    }
} catch {
